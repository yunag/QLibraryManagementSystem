...
public:
   ...
   * \param rect The geometry of the rating. The alignment of the rating is used
   * relative to this value. \param rating The actual rating value to draw.
   * \param hoverRating The hover rating indicates the position the user hovers
   * the mouse pointer at. This will provide visual feedback about the new
   * rating if the user would actually click as well as the difference to the
   *                    current rating.
   */
  void paint(QPainter *painter, const QRect &rect, int rating,
             int hoverRating = -1) const;
   ...
   * Qt::AlignJustify will insert spacing between the stars.
   */
