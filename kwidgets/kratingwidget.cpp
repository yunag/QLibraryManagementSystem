...
KRatingWidget::KRatingWidget(QWidget *parent)
    : QFrame(parent), d(new KRatingWidgetPrivate()) {
...
static inline int adjustedHoverRating(bool halfStep, int hoverRating,
                                      int rating) {
  ...
  // See bug 171343, if we click on a star we want it to be full star, click
  // again make it half, click third time make it clear.
...
QSize KRatingWidget::sizeHint() const {
  ...
  if (!d->ratingPainter.customPixmap().isNull()) {
    pixSize = d->ratingPainter.customPixmap().size() /
              d->ratingPainter.customPixmap().devicePixelRatio();
  }
